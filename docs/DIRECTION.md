...
- **🔲 노드 클릭 종착지** ([discovery-log.md](./discovery-log.md)의 `node-click-destination` = parked):
  change 클릭 시 무엇이 열리나. 후보 —
  - 2-a: 기존 5종 뷰 그대로
  - 2-b: 유저플로우만
  - 2-c: capability 패널에서 멈춤
  → **정적 HTML 목업 3개를 비교한 뒤 결정 예정.** 예광탄 구현 전에 필요.
...
**예광탄: 프로젝트 1개로 [카드 → 뼈대 그래프(한글) → capability 클릭 → change drill-down] 세로 한 줄 관통.**

단, 그 전에 **§5의 노드클릭(2번) 목업 비교**가 필요하다 — 종착지가 정해져야 change drill-down의 끝을 구현할 수 있다.
