...
프로젝트 카드 드릴다운으로 진입한 change의 5종 뷰(graph·ia·wireframe·prd·spec-tree)와 layout 저장을 그 프로젝트의 openspec/changes 하위에서 해석하는 능력. 선택적 `?project=` 쿼리(부재=글로벌 루트, 하위호환 100%)와 화이트리스트 검증(영숫자·하이픈·언더스코어, dotfile·traversal 차단)을 포함한다.
