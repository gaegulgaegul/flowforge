...
타 프로젝트(`PROJECTS_ROOT` 하위) change 의 유저플로우 그래프 레이아웃 저장이 HTTP 500 으로 실패했다. 호스트 홈이 읽기 전용으로 마운트돼 있는데(무인증 노출이라 의도적으로 잠근 보안 경계) 오버레이를 그 하위 `<changeDir>/viz/` 에 쓰려 했기 때문이다. 사용자가 UI 에서 노드를 드래그하면 실제로 밟히는 경로였다.

이 capability 는 크로스프로젝트 레이아웃을 전용 RW 볼륨(`OVERLAY_ROOT`)에 저장·재조회한다. 홈 마운트는 읽기 전용 그대로 두고(보안 경계 보존), 쓰기 불가 대상은 generic 500 이 아니라 진단 가능한 상태 코드로 거부한다. `WIREFRAME_FEEDBACK_ROOT` 가 동일한 읽기전용-홈 문제를 푼 선례와 같은 패턴이다.
