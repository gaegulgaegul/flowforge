# korean-display-labels Specification

## Purpose

화면에 보이는 표시명만 한글로 변환하고 capability·change·라우팅 키는 영문 슬러그를 그대로 유지하는 capability. capability 한글명은 출처1(spec.md `## capability: 키 — 한글` 병기)→출처2(키→한글 맵)→영문키 순으로, change 한글명은 출처3(proposal 제목)→영문키 순으로 해석한다.
