# flowforge-screen-crosslink

## Purpose

유저플로우 화면(page) 노드를 선택하면, 그 화면 id를 조인키로 (a)대응 와이어프레임 프리뷰/딥링크와 (b)연관 기능명세 상세기능 목록을 상호참조로 표시한다. 화면 id는 유저플로우·IA·와이어·기능명세가 공유하는 기존 조인키를 활용하며, 연결이 없거나 화면 노드가 아니면 빈 상태로 안전하게 처리한다(읽기 전용 상호참조). 피드백9(와이어 안 보임)·피드백12(기능 연관 안 보임)를 해소한다.
