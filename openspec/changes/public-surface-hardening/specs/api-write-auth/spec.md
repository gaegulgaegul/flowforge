...
- **Red**: (1) cfAccess 단위 — 픽스처 RSA 키쌍으로 서명한 JWT의 유효/위조/만료/aud 불일치/email 부재 판정, JWKS 캐시·kid 재조회(fetch는 주입 seam) (2) 라우트 통합 — env 설정 시 무자격 쓰기 401·문서 불변, 토큰 통과, 미설정 시 현행.
- **Green**: `lib/cfAccess.ts` + `requireWriteAuth` 미들웨어 + 5종 부착 + cors 교체.
- **Refactor**: 없음.
- Mock 대상: JWKS fetch(주입 seam — 실 네트워크 금지).
