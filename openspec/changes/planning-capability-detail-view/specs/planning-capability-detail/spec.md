...
- **Red (server)**: `capabilityIndex.test.ts`에 종합 함수 테스트 추가 — (a) features 서브트리가 일치 capability 가지만 포함 (b) 유저플로우가 `> capability:` 마커로만 연결 (c) change 목록이 `byCapability`와 동일 (d) 연결 0개면 빈 구조. 임시 디렉토리 픽스처(`makeChange` + features.md/user-flow 파일) 사용.
- **Red (route)**: `projects.test.ts`에 supertest로 `GET /api/projects/:project/capabilities/:cap` — 200 구조, 경로조작 4xx, 비프로젝트 4xx.
- **Green**: `capabilityIndex.ts`에 종합 뷰모델 함수(순수, 절대경로 주입식) + `projects.ts` 라우트(기존 `indexFor`/`resolveProjectDir`/`safe()` 재사용). features는 `buildDocsPlanningFeatures` 필터, 유저플로우는 `listDocsUserFlows` + 마커 스캔.
- **Refactor**: 미사용 `CapabilityNode` 타입을 종합 뷰모델 타입의 출발점으로 정리(구조 변경은 별도 커밋).
- **Mock 대상**: 없음(파일시스템 임시 픽스처로 실제 파일 읽기, `*Client`/`*Gateway` 외부 의존성 없음).
