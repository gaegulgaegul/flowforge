...
- [x] E.2 [frontend-agent] gstack(내부 ~/.cache/ms-playwright chromium)로 실픽셀 캡처. (1)planning-features-view 요구사항 노드에만 change 1 배지(e2-badge-on)·미연관 2노드 미표시(e2-badge-off) (2)서브트리 상속 4노드 배지·클리핑 0(e2-inherit·e4-tree-tab, DOM .feature-tree-change=4) (3)IA 역경유: ⚠️초기 캡처(e2-ia-screen)에서 배지 0 관찰→렌더 코드 부재로 판명(review 치명 1). IANode/IADetailPanel 렌더 구현(cf46759) 후 재검증(E.5)에서 IA 화면 노드(기능명세 화면·기획 뷰)에 change 1 배지 실렌더 확인(e5-ia-badge-on). 본체 직접 관찰(e2-inherit·e5-ia-badge-on)로 grounding
- [x] E.5 [frontend-agent] (review 치명 1 수정 후 재verify) gstack로 IA 화면 노드 역경유 배지·진입 실픽셀. IA "화면 구조" 탭에서 화면 노드 features·skeleton에 change 1 배지(e5-ia-badge-on), 미연관 화면(grid)·상세기능 노드 미표시(e5-ia-badge-off), 클릭→IADetailPanel 🔗 연관 change(1)→5종 뷰 PRD 탭+딥링크 URL 진입+패널 닫힘(e5-ia-detail-panel·e5-ia-open-views). spec Requirement 3 THEN 실증. 콘솔 에러 0
...
- [x] VERIFY: 5단계 게이트 전부 PASS — ①BUILD(shared·server·web tsc+vite build) ②TYPECHECK(3 워크스페이스 exit 0) ③LINT(lint:linkage 위반 0, 기획 cap 11 vs openspec 45 대조) ④TEST(server Jest 39스위트 459 passed, capabilityIndex 파생 포함·회귀 0) ⑤UI(gstack 17장 실픽셀 — FeatureTree 배지 표시/미표시/서브트리 상속 4노드/상세패널 진입/딥링크 URL/읽기전용/전역목록 제거/탭 회귀 + IA 화면 노드 배지·진입(E.5 재verify, review 치명 1 수정 후), 본체 직접 관찰 3장 grounding). review 재검토 판정=배포 가능(치명 1 해소). ⚠️ UI 배지는 verify fixture(specs/planning-features-view)로 실증 — 실데이터 배지는 후속 change [[flowforge-mapping-basis-shift]]에서 실증(활성 change specs/dir↔features.md capability 정합·archive 제외 완화)
