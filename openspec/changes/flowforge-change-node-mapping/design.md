...
### D0. 사용자 확정 (2026-07-11) — 조인 방식 + capability 단위
사용자가 두 핵심 지점을 확정했다(디스코드, 권장안 채택):
- **지점 1 = 조인 방식(권장 1번)**: 기획 노드는 그대로 두고 연관 change를 노드에 in-place로 붙인다. 노드 종류를 "기획기능/change"로 나누는 판별 방식(새 데이터 모델 필요)은 채택하지 않는다.
- **지점 2 = capability(요구사항) 단위(권장 A번)**: 매핑 입도는 capability 단위. 상세기능은 상위 capability의 change를 상속받아 다소 넓게 붙는 것을 허용한다. change↔상세기능 정밀 매핑(원천 데이터 신설)은 후속 change로 분리(비파괴 확장 가능).
이 D0가 이 change의 스코프를 확정하는 상위 결정이다. 아래 D1~D5는 이 결정의 구체화다.
