...
- **Red**: (1) `featureTreeBuilder.test.ts` — features.md 픽스처를 3단 FeatureTree로 파싱(capability/priority/status 채워짐, 파일 없으면 null). (2) `routes/__tests__/docs.test.ts` — planning-features 라우트 200+tree, 파일없음 404, 경로조작 404.
- **Green**: `buildDocsPlanningFeatures(docsDir)` 구현(markdown 헤더 위계 파싱 + capability 주석/속성 정규식). 라우트 추가(resolveDocsDir 재사용). 전용 타입 `FeatureTree`/`FeatureNode` 신설. web `FeatureNode.tsx`/`featureTreeAdapter.ts`/`fetchDocsPlanningFeatures`/App.tsx 탭.
- **Refactor**: featureTreeAdapter는 specTreeAdapter의 dagre LR 레이아웃 로직을 내부 참고하되 features 전용으로 둠(공유 추출은 과함 — 분리 유지).
- **Mock 대상**: 없음. 임시 디렉토리 픽스처로 파일 시스템 직접 검증.
