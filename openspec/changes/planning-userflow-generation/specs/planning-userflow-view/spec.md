...
- **Red**: (1) `planningUserFlowBuilder.test.ts` — Mermaid 픽스처를 SpecGraph로 파싱(노드 모양→kind, 엣지·라벨, 파일 없으면 null). (2) `routes/__tests__/docs.test.ts` — GET planning-user-flow 200+graph+layout, PUT layout 저장 후 overlay 파일 기록·재조회 반영, 경로조작 4xx, 잘못된 body 4xx.
- **Green**: `buildDocsPlanningUserFlow(docsDir, group, version)`(Mermaid flowchart 정규식 파싱→SpecGraph). overlay 읽기/쓰기(lib/docs.ts에 추가, changes.ts viz 패턴 참고). GET/PUT 라우트(resolveDocsDir·isLayoutOverlay 재사용, group/version 화이트리스트). web fetch/save + App 배선(graphAdapter/SpecNode 재사용, 드래그→PUT).
- **Refactor**: overlay 읽기/쓰기는 changes.ts의 readOverlay/writeOverlay와 시그니처가 유사하나 경로가 다름(viz/ vs user-flow/) — 공통 추출은 과하면 하지 않음(경로만 다른 얇은 함수).
- **Mock 대상**: 없음. 임시 디렉토리 픽스처로 파일 시스템 직접 검증(읽기·쓰기 왕복).
