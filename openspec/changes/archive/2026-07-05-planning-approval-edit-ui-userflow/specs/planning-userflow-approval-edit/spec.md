...
- **Red**: userFlowDocs 단위 — 큐 읽기(부재/깨짐/무효 필터), 검증(from 부재·id 충돌·라벨 금지문자·중복 에지 멱등), append 위치(닫는 펜스 직전), self-roundtrip(기존 노드·에지 보존, 신규 에지 일치, **무력화 프로브**). 라우트 통합 — GET 빈 큐 200·목록, POST 승인 반영·반려 문서 불변·skipped 표면화·422·404(부재 프로젝트·경로조작·무효 stem).
- **Green**: shared 타입 → lib/userFlowDocs.ts → 라우트 2개 → web 패널·배선.
- **Refactor**: 6b-features와 중복되는 큐 IO·apply 골격은 무리해서 공통화하지 않는다(문서 구조가 달라 억지 추상화 위험 — 필요해지면 별 change).
- Mock 대상 없음(임시 픽스처 파일 IO).
