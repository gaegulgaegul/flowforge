...
- **Red**: `prdBuilder.buildPrd(changeDir)`가 proposal/design 섹션을 5개 PRD 섹션으로 매핑하는지 검증하는 테스트(섹션 존재/누락/일부 누락 케이스). 픽스처 = wowa `implement-ios-app`의 proposal/design 일부를 복제한 최소 마크다운.
- **Green**: 마크다운을 `## 헤더` 단위로 분할하는 순수 함수 + 헤더명→PRD섹션 매핑 테이블. 헤더 매칭은 공백·대소문자 유연.
- **Refactor**: 마크다운 섹션 분할 유틸을 spec-tree와 공유 가능하면 `parser/markdown.ts`로 추출(구조 변경 커밋 분리).
- **Mock 대상**: 없음(파일시스템 읽기는 픽스처 디렉토리로 실제 수행, 외부 클라이언트 없음).
