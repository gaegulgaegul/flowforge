...
- [ ] 1.1 GREEN: `shared/src/user-flow-suggestion-types.ts` 신설 — `UserFlowSuggestion { id; op: 'add-edge'; from: string; to?: string; newNode?: { id: string; label: string }; edgeKind: 'happy' | 'edgecase'; label?: string; rationale?: string }`(to/newNode는 정확히 하나), `UserFlowSuggestionQueue { version: 1; suggestions: readonly UserFlowSuggestion[] }`. index 배럴 export.
