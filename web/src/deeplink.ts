 ...
 *
 * 스킴: `?project=<project>&change=<change>&tab=<tab>` (tab ∈ prd|spec|flow|ia|wire).
...
// change 5종 뷰 탭. 화이트리스트를 이 한 곳에만 두고 App.tsx가 소비(타입 드리프트 방지).
// manyfast 파이프라인 순서: PRD → 기능명세서 → 유저플로우 → IA → 와이어프레임.
export const TABS = ["prd", "spec", "flow", "ia", "wire"] as const;
export type Tab = (typeof TABS)[number];

// 기본 탭(필수 파라미터는 있으나 tab이 없거나 화이트리스트 밖일 때의 폴백).
const DEFAULT_TAB: Tab = "prd";

/** 문자열이 화이트리스트 5종 탭 중 하나인지 좁힘. */
 ...
 * - tab이 화이트리스트 5종이 아니면 "prd"로 정규화(알 수 없는 tab 방어).
